...
impl Model {
    pub async fn load_model(
        ...
        for m in obj_materials? {
            let diffuse_texture = Texture::load_texture(&m.diffuse_texture.unwrap(), device, queue,false).await?;
            let normal_texture = Texture::load_texture(&m.normal_texture.unwrap(), device, queue,true).await?;
